...
test("test for delete function", t => {
  ...
  const expected1 = [{ id: -3, description: "first todo", done: true }];
...
var newarray = [
  ...
  { id: 7, description: "seven todo", done: false },
  { id: 1, description: "first todo", done: false }
];
test("test for Sort Function", t => {
  const clonednewarray = [...newarray];
  const actual = logic.sortTodos(newarray, sort); // exe
  const expected = [
    { id: 7, description: "seven todo", done: false },
    { id: 4, description: "fourth todo", done: false },
    { id: 1, description: "first todo", done: false }
  ];
  t.deepEqual(actual, expected, "sort function works in decesnding order");
  t.deepEqual(newarray, clonednewarray, "check for pure function ");
  t.end();
});
